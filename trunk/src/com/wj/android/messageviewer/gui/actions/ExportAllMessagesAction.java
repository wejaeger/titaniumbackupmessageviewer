...
public class ExportAllMessagesAction extends AbstractAction
{
   ...
   /**
    * Constructs a new {@code ExportAllMessagesAction}.
    *
    * @param frame application main window
    * @param threadListBox list of currently loaded threads
    */
   public ExportAllMessagesAction(final JFrame frame, final JList<MessageThread> threadListBox)
   {
      putValue(Action.NAME, "Export All messages ...");
   ...
   public void actionPerformed(final ActionEvent e)
   {
      if (m_ThreadListBox.isEnabled())
      {
         if (m_ThreadListBox.getSelectedIndex() != -1)
         {
            ...
            if (iRet == 0)
               new ExportWorker(m_Frame, true, m_SaveChooser.getSelectedFile(), m_ThreadListBox).execute();
         }
         else
