...
<xsl:stylesheet  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  extension-element-prefixes="str redirect">
...
<xsl:param name="excludeFromCycles"/>
<xsl:variable name="names2Exclude" select="str:tokenize($excludeFromCycles)"/>
...
<xsl:template match="JDepend" mode="cycles.details">
  ...
  <xsl:if test="count(Cycles/Package[not($names2Exclude[text()] = Package[text()] or $names2Exclude[text()] = @Name)]) = 0">
    <p>There are no cyclic dependancies.</p>
  </xsl:if>
  <xsl:for-each select="Cycles/Package">
    <xsl:if test="not($names2Exclude[text()] = Package[text()] or $names2Exclude[text()] = @Name)">
      ...
      <h2>Cycles</h2>
        <table width="100%">
            <xsl:for-each select="Package">
               <xsl:sort select="@Name"/>
               <xsl:if test="not($names2Exclude[text()] = Package[text()] or $names2Exclude[text()] = @Name)">
                  <xsl:apply-templates select="." mode="all.cycles"/>
               </xsl:if>
            </xsl:for-each>
        </table>
    </body>
